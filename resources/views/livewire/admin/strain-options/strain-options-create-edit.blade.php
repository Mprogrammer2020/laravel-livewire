...
<div class="kt-subheader kt-grid__item" id="kt_subheader">
							<div class="kt-subheader__main">
								...
								{{$edit ? 'Edit' : 'Create'}} Strain Option</h3>
...
<div class="kt-portlet kt-portlet--mobile">
	...
	<div class="form-group col-lg-6">
			<label class="label-required">Select an effect</label>
			<!-- <input type="text" class="form-control" placeholder="Enter category name" name="name"> -->
           <select wire:model="desired_effect_id"  class="form-control @error('desired_effect_id') is-invalid @enderror">
		   <option value="">Select an effect</option>
		   @foreach($desiredEffects as $desiredEffect)
				<option value="{{$desiredEffect->id}}">{{$desiredEffect->name}}</option>
		   @endforeach
		   </select>
			@error('desired_effect_id')
	...
	<div class="form-group col-lg-6">
			...
			<!-- <input type="text" class="form-control" placeholder="Enter category name" name="name"> -->
            <input type="text" wire:model="name" class="form-control @error('name') is-invalid @enderror" placeholder="Strain Option Name *" autocomplete="off">
			@error('name')
			<div class="invalid-feedback">{{ $message }}</div>
			@enderror
		</div>
	...
	<div class="kt-form__actions">
		<div class="row">
			<div class="col-lg-6">
				...
				<a href="{{route('strain.options.list')}}" class="btn btn-secondary">Cancel</a>
			</div>
