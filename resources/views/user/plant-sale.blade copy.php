...
.c_error{
    ...
    <div class="layerThree">
        <div class="buyPlatBox bg-white">
            <div class="h4 text-primary font-weight-bold text-center mb-4">Buy Plants</div>
            <div class="row">
                <div class="col-xl-4">
                    <ul class="list-unstyled">
                        @forelse($allPlants as $plant)
                            <li class="d-block @if ($plant->id == $selectedPlant->id) active @endif">
                                <a href="{{ route('user.plant.sale', ['plant_id' => $plant->id]) }}"
                                    class="d-flex @if ($plant->id == $selectedPlant->id) active @endif align-items-center">
                                    <img src="{{ asset('uploads') . '/' . $plant->image }}" alt="">
                ...
                <div class="col-xl-8">
                    <div class="bigProd h-100 mx-0 row">
                        <div class="col-md-6 px-0 cursor-pointer" @click="setAsDefaultImage(default_image)">
                            <img :src="image" alt="" class="w-100 h-100">
                        </div>
                        <div class="col-md-6">
                            <div class="d-flex flex-column">
                                <div class="product_description">
                                    <div class="dt1">{{ $selectedPlant->name }}</div>
                                    {{-- <div class="dt2">Price: <span>${{ $selectedPlant->price }}</span></div> --}}
                                </div>
                                <div class="product_thumb">
                                    <div class="row mx-md-0 pt-3">
                                        @if($selectedPlant->image1)
                                        <div class="col-6 col-sm-3 col-md-6 px-0  cursor-pointer" @click="setAsDefaultImage(image1)">
                                            <img :src="image1" alt=""
                                                class="w-100 h-100">
                                        </div>
                                        @endif @if($selectedPlant->image2)
                                        <div class="col-6 col-sm-3 col-md-6 px-0 cursor-pointer" @click="setAsDefaultImage(image2)">
                                            <img :src="image2" alt=""
                                                class="w-100 h-100">
                                        </div>
                                        @endif
                                         @if($selectedPlant->image3)
                                        <div class="col-6 col-sm-3 col-md-6 px-0 cursor-pointer" @click="setAsDefaultImage(image3)">
                                            <img :src="image3" alt=""
                                                class="w-100 h-100">
                                        </div>
                                        @endif
                                         @if($selectedPlant->image4)
                                        <div class="col-6 col-sm-3 col-md-6 px-0 cursor-pointer" @click="setAsDefaultImage(image4)">
                                            <img :src="image4" alt=""
                                                class="w-100 h-100">
                                        </div>
                                        @endif
        ...
        <div class="row">
            <div class="col-xl-6 mb-5 mb-xl-0">
                <div class="whatUGet h-100">
                    <div class="row mx-0">
                        ...
                        <div class="col-xl-5">
                            <img src="{{ asset('uploads') . '/' . $selectedPlant->image }}" alt="" class="" style="height: 170px;">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-6 mb-5 mb-xl-0">
                <div class="orderYourPlant h-100">
                    ...
                    <button type="submit"  class="btn buyBtn-custom" name="submit" style="min-width:146px; min-height:40px; border-radius: 4px;border-style: none;background-color: #0f3b21;" alt="Pay with BtcPay, Self-Hosted Bitcoin Payment Processor"><span style="color:#fff">Buy Now</span>
                    </button></form>
 ...
 <script>if(!window.btcpay){    var head = document.getElementsByTagName('head')[0];   var script = document.createElement('script');   script.src='https://btcpay.gentlemencannabis.co/modal/btcpay.js';   script.type = 'text/javascript';   head.append(script);}function onBTCPayFormSubmit(event){    var xhttp = new XMLHttpRequest();    xhttp.onreadystatechange = function() {        if (this.readyState == 4 && this.status == 200) {            if(this.status == 200 && this.responseText){                var response = JSON.parse(this.responseText);                window.btcpay.showInvoice(response.invoiceId);            }        }    };    xhttp.open("POST", event.target.getAttribute('action'), true);    xhttp.send(new FormData( event.target ));}</script><style type="text/css"> .btcpay-form { display: inline-flex; align-items: center; justify-content: center; } .btcpay-form--inline { flex-direction: row; } .btcpay-form--block { flex-direction: column; } .btcpay-form--inline .submit { margin-left: 15px; } .btcpay-form--block select { margin-bottom: 10px; } .btcpay-form .btcpay-custom-container{ text-align: center; }.btcpay-custom { display: flex; align-items: center; justify-content: center; } .btcpay-form .plus-minus { cursor:pointer; font-size:25px; line-height: 25px; background: #DFE0E1; height: 30px; width: 45px; border:none; border-radius: 60px; margin: auto 5px; display: inline-flex; justify-content: center; } .btcpay-form select { -moz-appearance: none; -webkit-appearance: none; appearance: none; color: currentColor; background: transparent; border:1px solid transparent; display: block; padding: 1px; margin-left: auto; margin-right: auto; font-size: 11px; cursor: pointer; } .btcpay-form select:hover { border-color: #ccc; } #btcpay-input-price { -moz-appearance: none; -webkit-appearance: none; border: none; box-shadow: none; text-align: center; font-size: 25px; margin: auto; border-radius: 5px; line-height: 35px; background: #fff; } #btcpay-input-price::-webkit-outer-spin-button, #btcpay-input-price::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; } </style>
	...
	el: '#plant_sale_id',
	data: {
        default_image:"{{ asset('uploads') . '/' .$selectedPlant->image }}",
        image:"{{ asset('uploads') . '/' .$selectedPlant->image }}",
        image1:"{{ asset('uploads') . '/' .$selectedPlant->image1 }}",
        image2:"{{ asset('uploads') . '/' .$selectedPlant->image2 }}",
        image3:"{{ asset('uploads') . '/' .$selectedPlant->image3 }}",
        image4:"{{ asset('uploads') . '/' .$selectedPlant->image4 }}",
        plant_price: "{{ $selectedPlant->price }}",
        plant_want_to_buy:1,
